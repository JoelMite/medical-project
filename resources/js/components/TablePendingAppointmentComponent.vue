                              ...
                              Ver
                                ...
                                <form v-if="role_doctor == true" class="d-inline-block">
                                    <button class="btn btn-sm btn-success m-1" type="submit"
                                        @click.prevent="postConfirmAppointment(appointments)" data-toggle="tooltip"
                                ...
                                <form class="d-inline-block">
                                    <button class="btn btn-sm btn-danger m-1" type="submit"
                                        @click.prevent="postCancelAppointment(appointments)" data-toggle="tooltip"
                                        title="Cancelar cita">
    ...
    methods: {
        ...
        postConfirmAppointment(appointment) {
            ...
            axios.post('appointment_medicals/' + appointment.id + '/confirm').then(response => {
                this.getAppointments();
                this.emitter.emit('pending_confirm');
                $.notify({
        ...
        postCancelAppointment(appointment) {
            axios.post('appointment_medicals/' + appointment.id + '/cancel').then(response => {
                this.getAppointments();
                this.emitter.emit('pending_cancel');
                $.notify({
                    title: '<strong>Exito!</strong><br>',
